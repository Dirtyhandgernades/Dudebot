...
function fixture() {
  return {version:3,profile:'firm_first',feed:'sip',delay_minutes:16,send_at:new Date(target).toISOString(),generated_at:new Date(target-90_000).toISOString(),
    items:[{status:'QUALIFIED',ticker:'TEST',is_acquisition_corp:false,classification_evidence:true,exchange:'XNAS',security_type:'CS',firm_matches:1,
      ...
      signal_side:'SHORT',tradable:true,shortable:true,borrow_status:'easy_to_borrow',
...
test('hard exclusions and stale data suppress delivery',()=>{
  for(const change of [{ticker:'ABCDE'},{is_acquisition_corp:true},{classification_evidence:false},{halt_status:'UNKNOWN'},
    {price:3},{market_cap:24_999_999},{market_cap:null},{corporate_action_review:true},{firm_matches:0},{borrow_status:'hard_to_borrow'},{shortable:false},{halt_checked_at:'2026-09-08T18:00:00Z'},{price_time:'2026-09-08T18:00:00Z'}]) {
