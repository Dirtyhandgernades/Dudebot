...
class Scanner:
    def __init__(self,market,halts,cfg,entities,store):
    ...
    def scan(self,candidates,now):
        results=[]
        for c in candidates:
            r=structural(c,self.cfg,self.entities,now)
            if r.status!='STRUCTURAL_MATCH':results.append(r);continue
            halt=self.halts.check(c.ticker,now)
            if halt.status!='CLEAR':results.append(evaluate(c,self.cfg,self.entities,now,halt=halt));continue
            try:
                ...
                r=evaluate(c,self.cfg,self.entities,now,snapshot,halt)
